(function ($) {
  $.fn.typewrite = function (options) {
    ...
    function remove(remove) {
      var blankstr = new Array(remove.num || 0 + 1).join(" ");
      var chars = blankstr.split("");
      ...
      if (removeType === "whole") {
        $(settings.el)
          ...
          .queue(function (next) {
            ...
            if ((remove.from || remove.from === 0) && remove.to) {
              currText = $(this).text().substring(0, remove.from);
              currText += $(this).text().substring(remove.to);
            } else {
              var currText = currText.substring(
                0,
                $(this).text().length - remove.num
              );
            }
            $(this).html(currText.replace(/ \n /g, "<br>"));
            next();
